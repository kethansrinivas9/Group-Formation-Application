...
public class LoginImplementation implements LoginDao {
	...
	public Boolean getUserDetails(String id, String firstName, String email, String password) throws SQLException{
		...
		try {
                ...
                final String query = String.format("SELECT u.Password, r.RoleName,cc.CourseName from Users AS u INNER JOIN CourseRole AS c on (u.BannerID=c.BannerID) INNER JOIN Role AS r on (c.RoleID = r.RoleID) INNER JOIN Courses as cc on (c.CourseID = cc.CourseID) where u.BannerID ='%s' and u.Password='%s' ", id, password);
                ResultSet resultSet = connection.getRecords(query);
                while(resultSet.next()) {
                password_temp=resultSet.getString("u.Password");
                roles.add(new Role(resultSet.getString("r.RoleName"),resultSet.getString("cc.CourseName")));
                }
                resultSet.close();
                connection.close();
